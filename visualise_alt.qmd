---
title: "Visualise, only news articles"
format: html
editor: visual
---
...
```{r}
articles_df_alt <- articles_df |>
  filter(is_nyhed == 1)


total_articles_per_media <- articles_df_alt |>
  group_by(media) |>
  summarise(total_articles = n(), .groups = 'drop')


country_analysis <- articles_df_alt |>
